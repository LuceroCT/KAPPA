...
/*
robot::set_posx(int x){
  posx = x;
}

robot::mover(int x, int y){
  if(posx != x ){
    if (posx < x){
      set_posx(posx++);
    }
...
*/
//actualizar matriz de objeto almacen
...
void imprimircasilla(vector<casilla> sujeto,int dimension1, int dimension2){
  ...
  for (int j=0;j<dimension1;j++){
    for (int k=0;k<dimension2;j++){
      checar.push_back(k);
      checar.push_back(dimension1-j);
      auto iter = find(casillatovecc(sujeto).begin(),casillatovecc(sujeto).end(),checar);
